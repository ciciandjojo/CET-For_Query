...
def send_query_until_true(num):
    ...
    # 执行查询操作
    data = {
        "data": "CET4_181_DANGCI,{id},{name}".format(id=new_id, name=name),#看您是四级还是六级考试的修改为CET4_181还是CET6_181
        "v": code
    }
    log_info(data)
...
def main():
    for num in range(4554, 5000):# 这里的话根据你的考场号来决定大小，因为运行的好慢建议尽量缩小范围，还有不要用国内的ip
        if num%100 < 60:
            query_text = send_query_until_true(num)
